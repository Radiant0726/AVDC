...
def process_audio_info(conversations: list[dict] | list[list[dict]], use_audio_in_video: bool):
    ...
    for conversation in conversations:
        for message in conversation:
            ...
            for ele in message["content"]:
                if ele["type"] == "audio":
                    if ("audio" in ele and ele["audio"]) or ("audio_url" in ele and ele["audio_url"]):
                    ...
                    if ("video" in ele and ele["video"]) or ("video_url" in ele and ele["video_url"]):
    ...
    if len(audios) == 0:
        silent_audio = np.zeros(int(SAMPLE_RATE * 2), dtype=np.float32)
        audios = [silent_audio]
        # audios = None
    return audios
