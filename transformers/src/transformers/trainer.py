...
class Trainer:
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if labels is not None:
                ...
                self.accelerator.print(f"Before pad, rank {self.accelerator.local_process_index}, labels.shape = {labels.shape}")
    ...
    def create_accelerator_and_postprocess(self):
        ...
        if self.args.deepspeed_plugin:
            accelerator_config["dispatch_batches"] = False # 每个进程独立拉取流式数据
        if is_accelerate_available("0.28.0"):
