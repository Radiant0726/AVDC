...
def temporal_patch_avg(lst_lst, patch_size):
    out = []
    for lst in lst_lst:
        assert len(lst) % patch_size == 0, "长度必须是 patch_size 的倍数"
        out_one = []
        for i in range(0, len(lst), patch_size):
            patch = lst[i:i+patch_size]
            out_one.append(sum(patch)/patch_size)
        out.append(out_one)
    return out

class Qwen2_5OmniProcessor_FT(ProcessorMixin):
    r"""
    Constructs a Qwen2.5Omni processor.
    [`Qwen2_5OmniProcessor_FT`] offers all the functionalities of [`Qwen2VLImageProcessor`], [`WhisperFeatureExtractor`], and [`Qwen2TokenizerFast`]. See the
    [`~Qwen2_5OmniProcessor_FT.__call__`] and [`~Qwen2_5OmniProcessor_FT.decode`] for more information.
    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen2_5OmniProcessorKwargs],
        ...
        if videos is not None:
            ...
            videos_inputs["pixel_values_videos"] = videos_inputs["pixel_values_videos"].unsqueeze(0)
            # fps = [fps] * len(videos)
            videos_inputs["video_second_per_grid"] = [
                self.video_processor.temporal_patch_size / fps[i] if fps[i]!=0 else 0.0 for i in range(len(fps))
            ]
            video_timestamps = temporal_patch_avg(video_timestamps, self.video_processor.temporal_patch_size)
        ...
        # text, video_token_indices_lst, audio_token_indices_lst = self.replace_multimodal_special_tokens_timestamps(
        #     text,
        #     audio_lengths,
        #     image_grid_thw,
        #     video_grid_thw,
        ...
        # token_indices_inputs = {"video_token_indices":video_token_indices_lst, "audio_token_indices":audio_token_indices_lst}
        
        return BatchFeature(
            data={**texts_inputs, **images_inputs, **videos_inputs, **audio_inputs}, #, **token_indices_inputs
            ...
            for _, special_token in positions:
                if special_token == self.audio_token:
                    ...
                    if not use_audio_in_video:
                        ...
                        curr_video_second_per_grid = next(video_second_per_grid)
                        video_token_indices = (
                            video_token_indices * curr_video_second_per_grid * position_id_per_seconds
                        )   
                        # video_token_indices = (
                        #     video_token_indices * next(video_second_per_grid) * position_id_per_seconds
                        # )
                        # curr_video_second_per_grid = max(round(curr_video_second_per_grid / 2) * 2, 2)  if round(curr_video_second_per_grid)>=2 else max(round(curr_video_second_per_grid), 1)
                        # tokens_per_chunk = int(position_id_per_seconds * curr_video_second_per_grid)
                        ...
                        assert all(v[1]-v[0] > 2 for v in video_chunk_indexes) # check 分块
    ...
    def replace_multimodal_special_tokens_timestamps(
        self,
        text,
        audio_lengths,
        image_grid_thw,
        ...
        processed_text = []
        video_token_indices_lst, audio_token_indices_lst = [], []
        for sample in text:
            positions = []
            special_tokens = [re.escape(tok) for tok in [self.audio_token, self.image_token, self.video_token]]
            pattern = "|".join(special_tokens)
            ...
            for _, special_token in positions:
                if special_token == self.audio_token:
                    # sample = sample.replace(self.audio_token, "<|audio_placeholder|>" * next(audio_lengths), 1)
                    audio_token_indices = np.arange(next(audio_lengths))
                    video_token_indices = np.zeros_like(audio_token_indices)
                    tokens_per_chunk = int(position_id_per_seconds * seconds_per_chunk)
                    audio_chunk_indexes = self.get_chunked_index(audio_token_indices, tokens_per_chunk)
                    placeholder_string = ""
                    for j in range(len(audio_chunk_indexes)):
                        # timestamps
                        audio_timestamp_str = f"audio timestamp: from {seconds_per_chunk*j} to {seconds_per_chunk*(j+1)} seconds."
                        placeholder_string += audio_timestamp_str
                        # features
                        audio_seq_length = audio_chunk_indexes[j][1] - audio_chunk_indexes[j][0]
                        placeholder_string += "<|audio_placeholder|>" * audio_seq_length
                    sample = sample.replace(self.audio_token, placeholder_string, 1)
                elif special_token == self.image_token: # 合并到video
                    image_seq_length = next(image_grid_thw).prod() // merge_length_image
                    sample = sample.replace(self.image_token, "<|image_placeholder|>" * image_seq_length, 1)
                    audio_token_indices = np.zeros(image_seq_length)
                    video_token_indices = np.zeros(image_seq_length)
                elif special_token == self.video_token:
                    if not use_audio_in_video:
                        # video_seq_length = next(video_grid_thw).prod() // merge_length_video
                        # sample = sample.replace(self.video_token, "<|video_placeholder|>" * video_seq_length, 1)
                        ...
                        audio_token_indices = np.zeros_like(video_token_indices)
                        curr_video_second_per_grid = max(round(curr_video_second_per_grid / 2) * 2, 2)  if round(curr_video_second_per_grid)>=2 else max(round(curr_video_second_per_grid), 1)
                        tokens_per_chunk = int(position_id_per_seconds * curr_video_second_per_grid)
                        video_chunk_indexes = self.get_chunked_index(video_token_indices, tokens_per_chunk)
                        curr_video_timestamps = next(video_timestamps)
                        # assert len(curr_video_timestamps) == len(video_chunk_indexes)
                        if len(curr_video_timestamps) != len(video_chunk_indexes):
                            raise Exception(
                                f"Length mismatch: curr_video_timestamps={len(curr_video_timestamps)}, "
                                f"video_chunk_indexes={len(video_chunk_indexes)}"
                            )
                        placeholder_string = ""
                        for j in range(len(video_chunk_indexes)):
                            # timestamps
                            video_timestamp_str = f"video timestamp: at {round(float(curr_video_timestamps[j]), 2)} seconds."
                            placeholder_string += video_timestamp_str
                            # features
                            video_seq_length = video_chunk_indexes[j][1] - video_chunk_indexes[j][0]
                            placeholder_string += "<|video_placeholder|>" * video_seq_length
                        sample = sample.replace(self.video_token, placeholder_string, 1)
                    else:
                        audio_token_indices = np.arange(next(audio_lengths)) # audio_lengths = (second * sr=16000 /4 // hop_length=160), 2s-50, 4s-100
                        curr_video_grid_thw = next(video_grid_thw)
                        height = curr_video_grid_thw[1] // self.video_processor.merge_size
                        width = curr_video_grid_thw[2] // self.video_processor.merge_size
                        video_token_indices = np.arange(curr_video_grid_thw[0]).reshape(-1, 1, 1)
                        ...
                        for j in range(max(len(video_chunk_indexes), len(audio_chunk_indexes))):
                            # timestamps
                            if j < len(video_chunk_indexes):
                                video_timestamp_str = f"video timestamp: {round(float(curr_video_timestamps[j]), 2)} seconds. "
                                placeholder_string += video_timestamp_str
                            if j < len(audio_chunk_indexes):
                                # audio_timestamp_str = f"audio timestamp: from {seconds_per_chunk*j} to {seconds_per_chunk*(j+1)} seconds."
                                audio_timestamp_str = f"audio timestamp: from {curr_video_second_per_grid * j} to {curr_video_second_per_grid * (j+1)} seconds."
                                placeholder_string += audio_timestamp_str
                            # features
                            if j < len(video_chunk_indexes):
                                video_seq_length = video_chunk_indexes[j][1] - video_chunk_indexes[j][0]
            ...
            video_token_indices_lst.append(video_token_indices)
            audio_token_indices_lst.append(audio_token_indices)
        ...
        return processed_text, video_token_indices_lst, audio_token_indices_lst
    ...
    def merge_video_intervals(self, pairs: list[tuple[int, int]], timestamps, min_len: int = 720):
        pairs = pairs[:]  # 拷贝一份，避免修改原始输入
        time_id = 0
        timestamps_pad = []
        for i in range(len(pairs) - 1):
            ...
            if (end - start) < min_len:
                ...
                pairs[i + 1] = (start, next_end)
                pairs[i] = -1
                timestamps_pad.append(-1)
            else:
                timestamps_pad.append(timestamps[time_id])
                time_id += 1
        timestamps_pad.append(timestamps[-1])
        return pairs, timestamps_pad
...
__all__ = ["Qwen2_5OmniProcessor_FT"]
