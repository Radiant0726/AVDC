            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(multimodal_nums):
                    ...
                    if min_ed == ed_audio:
                        ...
                        # t_ntoken_per_chunk = int(position_id_per_seconds * second_per_grids[video_idx])
...
class Qwen2_5OmniVisionEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        seq_len = torch.prod(grid_thw)
        hidden_states = hidden_states[:seq_len] # 去除pad的部分
    ...
    def forward(
        ...
        **kwargs
        ...
        causal_mask = self._update_causal_mask(
            attention_mask, inputs_embeds, cache_position, past_key_values, output_attentions, **kwargs
        )
        ...
        **kwargs
        ...
        if self.config._attn_implementation == "flash_attention_2":
            if attention_mask is not None and past_key_values is not None:
                ...
                # is_padding_right = attention_mask[:, 0].sum().item() == input_tensor.size()[0] if input_tensor.size()[0] !=1 else False
                if is_padding_right:
                    raise ValueError(
...
class Qwen2_5OmniThinkerForConditionalGeneration_FT(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def get_video_features(
        ...
        batch_video_embeds = []
        batch_pixel_values_videos, batch_video_grid_thw = pixel_values_videos, video_grid_thw
        for i in range(batch_pixel_values_videos.shape[0]):
            pixel_values_videos, video_grid_thw = batch_pixel_values_videos[i], batch_video_grid_thw[i]
            video_embeds = self.visual(pixel_values_videos, grid_thw=video_grid_thw.unsqueeze(0))
            batch_video_embeds.append(video_embeds)
        video_embeds = torch.cat(batch_video_embeds)
        return video_embeds
    ...
    def forward(
        ...
        **kwargs
        ...
        >>> from transformers import Qwen2_5OmniProcessor, Qwen2_5OmniThinkerForConditionalGeneration_FT

        >>> thinker = Qwen2_5OmniThinkerForConditionalGeneration_FT.from_pretrained("Qwen/Qwen2.5-Omni-7B")
        ...
        ```"""
        # print("use_audio_in_video:",use_audio_in_video)
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        if attention_mask is not None and position_ids is None:
            if (
                ...
                delta0 = delta0.to(rope_deltas.device)
...
class Qwen2_5OmniThinkerForConditionalGeneration_VCen_FT(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    config_class = Qwen2_5OmniThinkerConfig
    base_model_prefix = "thinker"
    _no_split_modules = ["Qwen2_5OmniAudioEncoder", "Qwen2_5OmniVisionEncoder"]
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        # self.CrossModalAttender = CrossModalAttender(fea_v=config.vision_config.out_hidden_size,
        #                                              fea_a=config.audio_config.output_dim,
        #                                              d_model=64)
        # self.tokenizer = AutoTokenizer.from_pretrained("/mnt/vision_user/kaiyinyan/models/Qwen2.5-Omni-7B", use_fast=False, trust_remote_code=True)
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        video_second_per_grid: Optional[torch.LongTensor] = None,
        # video_token_indices: Optional[torch.FloatTensor] = None,
        # audio_token_indices: Optional[torch.FloatTensor] = None,
        num_qa: Optional[torch.LongTensor] = None,
        **kwargs
    ) -> Union[Tuple, Qwen2_5OmniThinkerCausalLMOutputWithPast]:
        r"""
        input_features (`torch.FloatTensor` of shape `(batch_size, feature_size, feature_sequence_length)`):
        ...
        #     assert torch.min(num_qa) == torch.max(num_qa)
        #     split_input_ids = torch.split(input_ids, num_qa.tolist(), dim=0)
        #     input_ids = torch.stack([split_value[0] for split_value in split_input_ids], dim=0)
        ...
        if isinstance(use_audio_in_video, bool):
            use_audio_in_video = torch.tensor([use_audio_in_video] * inputs_embeds.shape[0]).to(inputs_embeds.device)
            
        # 2. Merge text , audios , image and video
        batch_audio_features = None
        batch_video_embeds = None
        if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
            if input_features is not None:
                audio_features, audio_output_lengths = self.get_audio_features(
                    input_features,
                ...
                audio_features = audio_features.to(inputs_embeds.device, inputs_embeds.dtype)
                batch_audio_features = list(torch.split(audio_features, audio_output_lengths.tolist(), dim=0))
                if num_qa is not None:
                    audio_features = torch.cat([bf.repeat(num, 1) for bf, num in zip(batch_audio_features, num_qa)], dim=0)
                inputs_embeds = inputs_embeds.masked_scatter(audio_mask, audio_features)
            ...
            if pixel_values_videos is not None:
                batch_video_embeds = self.get_video_features(pixel_values_videos, video_grid_thw)
                video_embeds = torch.cat(batch_video_embeds)
                video_mask = (
                    (input_ids == self.config.video_token_id)
                    .unsqueeze(-1)
                ...
                video_embeds = video_embeds.to(inputs_embeds.device, inputs_embeds.dtype)
                if num_qa is not None:
                    video_embeds = torch.cat([bf.repeat(num, 1) for bf, num in zip(batch_video_embeds, num_qa)], dim=0)
                inputs_embeds = inputs_embeds.masked_scatter(video_mask, video_embeds)
        ...
        features_dict = None
        # features_dict = self.get_features_chunk(batch_audio_features, batch_video_embeds,
        #                                         audio_token_indices, video_token_indices,
        #                                         video_second_per_grid)

        if attention_mask is not None and position_ids is None:
            if num_qa is not None:
                image_grid_thw = torch.repeat_interleave(image_grid_thw, num_qa, dim=0) if image_grid_thw is not None else None
                video_grid_thw = torch.repeat_interleave(video_grid_thw, num_qa, dim=0)  if video_grid_thw is not None else None
                audio_feature_lengths = torch.repeat_interleave(audio_feature_lengths, num_qa, dim=0) if audio_feature_lengths is not None else None
                video_second_per_grid = torch.repeat_interleave(video_second_per_grid, num_qa, dim=0) if video_second_per_grid is not None else None
                use_audio_in_video = torch.repeat_interleave(use_audio_in_video, num_qa, dim=0) if use_audio_in_video is not None else None
            ...
            if (
                ...
                delta0 = (1 - attention_mask).sum(dim=-1).unsqueeze(1)
                # position_ids, rope_deltas = self.get_rope_index_timestamps(
                position_ids, rope_deltas = self.get_rope_index(
                    input_ids,
                    image_grid_thw,
                    video_grid_thw,
    ...
    def get_features_chunk(self,batch_audio_features,
                                batch_video_embeds,
                                audio_token_indices,
                                video_token_indices,
                                video_second_per_grid,
        ...
        # contrast features
        features_dict = None
        if audio_token_indices is not None and video_token_indices is not None and batch_audio_features is not None and batch_video_embeds is not None:
            assert len(audio_token_indices) == len(video_token_indices) and len(audio_token_indices) == len(batch_audio_features) and len(video_token_indices) == len(batch_video_embeds)
            audio_features_chunk = []
            video_features_chunk = []
            video_segment_nums = []
            video_time_ids = []
            for k in range(audio_token_indices.shape[0]):
                curr_video_second_per_grid = max(round(video_second_per_grid[k].item() / 2) * 2, 2) if round(video_second_per_grid[k].item())>=2 else max(round(video_second_per_grid[k].item()), 1) 
                t_ntoken_per_chunk = int(self.config.position_id_per_seconds * curr_video_second_per_grid)
                audio_token_idx = audio_token_indices[k]
                video_token_idx = video_token_indices[k]
                audio_token_idx = audio_token_idx[audio_token_idx>=0]
                video_token_idx = video_token_idx[video_token_idx>=0]

                video_chunk_indexes = self.get_chunked_index(video_token_idx, t_ntoken_per_chunk, 0)
                audio_chunk_indexes = self.get_chunked_index(audio_token_idx, t_ntoken_per_chunk, 0)
                audio_chunk_indexes = audio_chunk_indexes[:len(video_chunk_indexes)] # 末尾舍弃
                if audio_chunk_indexes[-1][1] - audio_chunk_indexes[-1][0] < t_ntoken_per_chunk:
                    video_chunk_indexes = video_chunk_indexes[:-1]
                    audio_chunk_indexes = audio_chunk_indexes[:-1]

                video_chunk_time_ids = [video_token_idx[indexes[0]].item() for indexes in video_chunk_indexes]

                if len(video_chunk_indexes) > num_chunk: 
                    sample_idx = random.sample([i for i in range(len(video_chunk_indexes))], k=num_chunk)
                    video_chunk_indexes = [video_chunk_indexes[s] for s in sample_idx]
                    audio_chunk_indexes = [audio_chunk_indexes[s] for s in sample_idx]
                    video_chunk_time_ids = [video_token_idx[indexes[0]].item() for indexes in video_chunk_indexes]
                
                video_segment_nums.append(len(video_chunk_time_ids))
                video_time_ids.extend(video_chunk_time_ids)

                for (video_start, video_end), (audio_start, audio_end) in zip(video_chunk_indexes, audio_chunk_indexes):
                    # video_features_chunk.append(batch_video_embeds[k][video_start:video_end])
                    # audio_features_chunk.append(batch_audio_features[k][audio_start:audio_end])
                    video_features_chunk.append(batch_video_embeds[k][video_start:video_end].mean(dim=0))
                    audio_features_chunk.append(batch_audio_features[k][audio_start:audio_end].mean(dim=0))

            if len(audio_features_chunk)!=0 and len(video_features_chunk)!=0:
                audio_features_chunk = torch.stack(audio_features_chunk, dim=0)   # [N, D]
                video_features_chunk = torch.stack(video_features_chunk, dim=0)   # [N, D]
                # video_features_chunk, audio_features_chunk = self.CrossModalAttender(video_features_chunk, audio_features_chunk)
                
                features = torch.stack([audio_features_chunk, video_features_chunk], dim=0)  # [N, 2, D]
                video_segment_nums = torch.tensor(video_segment_nums).to(features.device)
                video_time_ids = torch.tensor(video_time_ids).to(features.device)
                features_dict = {"features":features, 
                                  "video_segment_nums": video_segment_nums, 
                                  "video_time_ids": video_time_ids}
        return features_dict
                
    def get_rope_index_timestamps(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        image_grid_thw: Optional[torch.LongTensor] = None,
        video_grid_thw: Optional[torch.LongTensor] = None,
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(multimodal_nums):
                    ...
                    if min_ed == ed_audio:
                        ...
                        audio_llm_pos_ids = torch.arange(audio_len).view(1, -1).expand(3, -1) + st_idx
                        t_ntoken_per_chunk = int(position_id_per_seconds * 2)
                        audio_chunk_indexes = self.get_chunked_index(audio_llm_pos_ids[0], t_ntoken_per_chunk, st_idx)
                        ...
                        for j in range(len(audio_chunk_indexes)):
                            ...
                            st_idx = llm_pos_ids_list[-1].max() + 1 if len(llm_pos_ids_list) > 0 else 0
                            ed_fea_chunk = input_tokens.index(audio_token_id, st)
                            timestamp_len = ed_fea_chunk - st
                            if timestamp_len > 30:
                                print(j)
                                raise ValueError
                            if timestamp_len != 0:
                                ...
                                st += timestamp_len
                                acc_timestamp_len += timestamp_len

                            st += audio_chunk_index[1] - audio_chunk_index[0]

                            llm_pos_ids_list.append(
                                audio_llm_pos_ids[:, audio_chunk_index[0] : audio_chunk_index[1]] + acc_timestamp_len
                            )
                        ...
                        curr_video_second_per_grid = max(round(second_per_grids[video_idx].item() / 2) * 2, 2)  if round(second_per_grids[video_idx].item())>=2 else max(round(second_per_grids[video_idx].item()), 1)
                        t_ntoken_per_chunk = int(position_id_per_seconds * curr_video_second_per_grid)
                        video_chunk_indexes = self.get_chunked_index(llm_pos_ids[0], t_ntoken_per_chunk, st_idx)
                        ...
                        for j in range(len(video_chunk_indexes)):
                            ...
                            st_idx = llm_pos_ids_list[-1].max() + 1 if len(llm_pos_ids_list) > 0 else 0
                            # timestamps
                            ed_fea_chunk = input_tokens.index(video_token_id, st)
                            timestamp_len = ed_fea_chunk - st
                            if timestamp_len > 30:
                                print(j)
                                raise ValueError
                            ...
                            st += video_chunk_index[1] - video_chunk_index[0]

                            llm_pos_ids_list.append(
                                video_llm_pos_ids[:, video_chunk_index[0] : video_chunk_index[1]] + acc_timestamp_len
                            )
                        ...
                        # text_len = min_ed - st - 2
                        text_len = vision_start_indices.item() - st
                        if text_len != 0:
                            st_idx = llm_pos_ids_list[-1].max() + 1 if len(llm_pos_ids_list) > 0 else 0
                            llm_pos_ids_list.append(torch.arange(text_len).view(1, -1).expand(3, -1) + st_idx)
                        ...
                        # t_ntoken_per_chunk = int(position_id_per_seconds * seconds_per_chunk)
                        # t_ntoken_per_chunk = int(position_id_per_seconds * round(second_per_grids[video_idx]))
                        curr_video_second_per_grid = max(round(second_per_grids[video_idx].item() / 2) * 2, 2)  if round(second_per_grids[video_idx].item())>=2 else max(round(second_per_grids[video_idx].item()), 1)
                        t_ntoken_per_chunk = int(position_id_per_seconds * curr_video_second_per_grid)
                        video_chunk_indexes = self.get_chunked_index(video_llm_pos_ids[0], t_ntoken_per_chunk, st_idx)
                        audio_chunk_indexes = self.get_chunked_index(audio_llm_pos_ids[0], t_ntoken_per_chunk, st_idx)
                        ...
                        acc_timestamp_len = 0
                        for j in range(max(len(video_chunk_indexes), len(audio_chunk_indexes))):
                            video_chunk_index = video_chunk_indexes[j] if j < len(video_chunk_indexes) else None # and video_chunk_indexes[j] != -1 
                            audio_chunk_index = audio_chunk_indexes[j] if j < len(audio_chunk_indexes) else None
                            st_idx = llm_pos_ids_list[-1].max() + 1 if len(llm_pos_ids_list) > 0 else 0
                            # timestamps
                            if video_chunk_index is not None: # video + audio
                                ed_fea_chunk = input_tokens.index(video_token_id, st)
                                timestamp_len = ed_fea_chunk - st
                                if timestamp_len > 30:
                                    print(j)
...
class CrossModalAttender(nn.Module):
    def __init__(self, fea_v, fea_a, d_model, num_heads=4):
        super().__init__()
        # 投影层，MultiHeadAttention 等
        self.v_proj = nn.Linear(fea_v, d_model)
        self.a_proj = nn.Linear(fea_a, d_model)
        self.cross_attn = nn.MultiheadAttention(embed_dim=d_model, num_heads=num_heads)

    def forward(self, V_feats, A_feats):
        # project
        Vp = self.v_proj(V_feats)   # (batch, Tv, D_model)
        Ap = self.a_proj(A_feats) # same shape
        # prepare for attention: maybe as queries = Vp, keys/values = Ap
        # 或者也做双向
        attn_out_v2a, _ = self.cross_attn(query=Vp.transpose(0,1),
                                          key=Ap.transpose(0,1),
                                          value=Ap.transpose(0,1))  
        # 返回 shape (Tv, batch, D_model) → transpose back
        attn_out_v2a = attn_out_v2a.transpose(0,1)
        # 可做 residual connection:
        V_cross = Vp + attn_out_v2a  # (batch, Tv, D_model)
        # 同样 Audio attend Vision (如果想):
        attn_out_a2v, _ = self.cross_attn(query=Ap.transpose(0,1),
                                          key=Vp.transpose(0,1),
                                          value=Vp.transpose(0,1))
        attn_out_a2v = attn_out_a2v.transpose(0,1)
        A_cross = Ap + attn_out_a2v

        V_cross, A_cross = torch.mean(V_cross, dim=1), torch.mean(A_cross, dim=1)
        return V_cross, A_cross
...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.thinker = Qwen2_5OmniThinkerForConditionalGeneration_FT(config.thinker_config)
...
__all__ = [
    ...
    "Qwen2_5OmniThinkerForConditionalGeneration_VCen_Contra_FT",
